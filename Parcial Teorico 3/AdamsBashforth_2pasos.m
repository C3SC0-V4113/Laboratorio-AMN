...
for j=a:h:b
   i=1+i;
   w(i+1)=w(i)+((h/2)*((3*(subs(f,{x,y},{t(i),w(i)})))-(subs(f,{x,y},{t(i-1),w(i-1)}))));
   fprintf('\n');
   fprintf('           - w%1.0f = w%1.0f + h/2 [3*f(t%1.0f,w%1.0f) - f(t%1.0f,w%1.0f)]',double(i),double(i-1),double(i-1),double(i-1),double(i-2),double(i-2));
   fprintf('\n');
   fprintf('           - w%1.0f = w%1.0f + %1.15f [3*f(%1.15f,w%1.0f) - f(%1.15f,w%1.0f)]',double(i),double(i-1),double(h/2),double(t(i)),double(i-1),double(t(i-1)),double(i-2));
   fprintf('\n');
end
