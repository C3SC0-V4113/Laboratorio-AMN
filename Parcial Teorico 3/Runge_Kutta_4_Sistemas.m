function w = Runge_Kutta_4_Sistemas(funciones, intervalo, h, valores_iniciales)
%RUNGE_KUTTA_4_SISTEMAS Sol. de sist. de ec. diferenciales por Runge-Kutta4
%   w = RUNGE_KUTTA_4_SISTEMAS(funciones, intervalo, h, valores_iniciales)
...
%       dU1 = U2;
%       dU2 = U3;
%       dU3 = exp(t) + 2*U1 + U2 - 2*U3;
%                            funciones f1,f2...,intervalo,h,condic. inicial
%       Runge_Kutta_4_Sistemas([dU1, dU2, dU3], [0, 0.1], 0.05, [1, 2, 0])
%
...
a = intervalo(1);       %Valor inicial y final
b = intervalo(2);
...
for j = 1:length(t)-1
    ...
    for i=1:m
        ...
        fprintf(', w%d%d + 1/2*K%d%d', [1:m; repmat(j-1, 1, m); repmat(1, 1, m); 1:m]);
        fprintf(') = %.15f\n', double(K(2, i)));
    end
    fprintf('\n');
    ...
    for i = 1:m
        ...
        fprintf('w%d%d = w%d%d + 1/6*(K1%d + 2*K2%d + 2*K3%d + K4%d) = %.15f\n', i, j, i, j-1, i, i, i, i, w(i, j+1));
    end
