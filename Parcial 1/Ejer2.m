...
X = input('Ingrese los valores de las w [w1 w2 ... wn]: ');
R = input('Ingrese la resistencia en ohmios (R): ');
C = input('Ingrese la Capacitancia en Faradios (C): ');
L = input('Ingrese la Inductancia en Henrys (L): ');
n = length(X);
%Duplicando
Z = 1:2*n;
k=1;
...
switch opc
    case 1
        f = input('Introduzca la función Z(x): ');
        %1/(sqrt((1/(R^2))+(w*C-(1/(w*L)))^2))
        fplot(@(w) (1/(sqrt((1/(R^2))+(w*C-(1/(w*L)))^2))),[148,156])
        for i=1:length(Z)
            D0(i)= subs(f,Z(i)); 
        end
        fdiff = diff(f,w); 
